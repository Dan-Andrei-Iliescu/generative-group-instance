...
\begin{abstract}
Group-instance disentanglement is the problem of learning separate representations for within-group and across-group variation.  The current state-of-the-art methods for solving this problem follow the paradigm of the Group Variational Autoencoder (GVAE).  In our work, we introduce a simple synthetic dataset to show that these methods fail to learn disentangled representations on datasets with ``emergent groups''; i.e.  data where the group and instance variables cannot be inferred from a single datapoint. To overcome this limitation,  we propose the Context-Aware Variational Autoencoder, a modified GVAE that 1) infers the instance variable conditional on the group variable, 2) has a more expressive group encoder, 3) optimizes a regularization objective that encourages independence between the instance variable and the grouping. We show empirically the impact of each modification with respect to holdout data likelihood, unsupervised translation, and other disentanglement metrics.
\end{abstract}
